...
def load_or_create_seed(seedpath):
    # 既にファイルがある → 読み込み
    if SEED_FILE in os.listdir():
        try:
            with open(seedpath, "rb") as f:
                seed = f.read()
            # 正しいSEEDが入っているかチェック
...
MASTER_SEED = load_or_create_seed(seedpath)
...
# ===== AES鍵生成 =====
def derive_aes_key(uid: bytes) -> bytes:
    raw = MASTER_SEED + uid
    return uhashlib.sha256(raw).digest()  # 32 bytes → AES-256鍵
...
def aes_encrypt(key: bytes, data: bytes) -> bytes:
    ...
    cipher = cryptolib.aes(key, 2, iv)  # AES-CBC
    encrypted = cipher.encrypt(padded)
    return iv + encrypted

# ===== AES-CBC 復号 =====
def aes_decrypt(key: bytes, enc: bytes) -> bytes:
    iv = enc[:16]
    ciphertext = enc[16:]
...
import uos
# ===== HMACキーのロードまたは自動生成 =====
def load_or_create_hmac_key(aes_key: bytes, size: int = 32) -> bytes:
    enc_data = load_file(HMAC_KEY_FILE)

    if enc_data:
        # ファイルが存在 → 復号
        return aes_decrypt(aes_key, enc_data)

    # 存在しない → 自動生成
    key = uos.urandom(size)

    enc = aes_encrypt(aes_key, key)
    save_file(HMAC_KEY_FILE, enc)

    return key

# ===== keypart のロードまたは自動生成 =====
def load_or_create_keypart(aes_key: bytes, size: int = 64) -> bytes:
    enc_data = load_file(KEYPART_FILE)
    ...
    # 存在しない → 自動生成
    keypart = uos.urandom(size)

    enc = aes_encrypt(aes_key, keypart)
    save_file(KEYPART_FILE, enc)

    return keypart

def initialize_hsm(uid: bytes):
    aes_key = derive_aes_key(uid)
    
    hmac_key  = load_or_create_hmac_key(aes_key)
    keypart   = load_or_create_keypart(aes_key)

    return hmac_key, keypart

# =========================================================
#  鍵読み込み
# =========================================================
SECRET_HMAC_KEY,SECRET_AES_KEYPART = initialize_hsm(RAW_UID)
